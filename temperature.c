...
unsigned char get_temperature_data_ctrl(p_temperature_data p_temp,unsigned int ADC_data)//在主函数中，每2.5秒处理一次,返回当前温度值
{
	...
	if(ADC_data<886)p_temp->ctrl = TemperatureCtrl(p_temp->set,p_temp->value);//热敏电阻值大于65K被视做开路损坏，否则程序计算将出现溢出错误。
	else            p_temp->ctrl =0;//热敏电阻出错。
	return p_temp->value;
}

void close_temperature_ctrl(p_temperature_data p_temp)//关闭发热功能,返回值为当前温度值
{
	p_temp->ctrl =  0;
	p_temp->count = 0;
}
...
unsigned char TemperatureCtrl(unsigned char data Temperature_Set,unsigned char data Temperature)
{
    ...
    {
             if((Temperature_Set-Temperature)==0)TemperatureCtrl_Data=1;//接通加热电源。
        else if((Temperature_Set-Temperature)<=2)TemperatureCtrl_Data=2;//接通加热电源。
